...
> **Assignment:** SDE-2 Frontend Challenge #2 — *Build a production-ready e-commerce platform.*
...
- **Checkout experience** — "Proceed to Checkout" launches a confetti celebration and an **order-confirmation dialog** showing a generated Order ID and estimated delivery. *(It's a demo checkout — no payment is taken and no real order is placed.)*
...
- **Persist orders & clear the cart on checkout.** Save an order history and empty the cart once an order is confirmed (today the confirmation is shown but the cart is left intact).
